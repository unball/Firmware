...
// 0 -> +/-250; 1 -> +/-500; 2 -> +/-1000; 3 -> +/-2000
    MPU6050_write_reg (MPU6050_GYRO_CONFIG, (0x02)<<3);
}
...
void imuRead(){
    ...
    double alpha = 0.9;
    values.x_accel = (alpha*values.x_accel) + (1-alpha)*accel_t_gyro.value.x_accel;
    values.y_accel = (alpha*values.y_accel) + (1-alpha)*accel_t_gyro.value.y_accel;
    values.z_accel = (alpha*values.z_accel) + (1-alpha)*accel_t_gyro.value.z_accel;
    values.x_gyro = (alpha*values.x_gyro) + (1-alpha)*accel_t_gyro.value.x_gyro;
    values.y_gyro = (alpha*values.y_gyro) + (1-alpha)*accel_t_gyro.value.y_gyro;
    values.z_gyro = (alpha*values.z_gyro) + (1-alpha)*accel_t_gyro.value.z_gyro;
